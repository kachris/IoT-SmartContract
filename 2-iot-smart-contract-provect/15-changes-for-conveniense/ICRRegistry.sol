...
contract ICRRegistry is ReentrancyGuard {
    ...
    error ICR__MCBalanceIsBelowMinimum(address _mc, uint256 _mcBalance);
    ...
    function registerCar(address _mc, uint256 _price) public nonReentrant {
        ...
        s_mcIsUsed[_mc] = true;
        Car memory car = Car(_mc, _price, Status.UNAVAILABLE, 0, i_owner);
    ...
    function getRentTime() public pure returns (uint256) {
        return RENT_TIME;
    }

    function getMinimumMcBalance() public pure returns (uint256) {
        return MINIMUM_MC_BALANCE;
    }
